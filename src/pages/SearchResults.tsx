        ...
        {searchAnalytics && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="font-semibold text-blue-800">AI Search Insights</span>
            </div>
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                <span className="text-blue-600">Indexed Products:</span>
                <span className="ml-2 font-semibold">{searchAnalytics.indexedProducts}</span>
              </div>
              <div>
                <span className="text-blue-600">Cache Size:</span>
                <span className="ml-2 font-semibold">{searchAnalytics.cacheSize}</span>
              </div>
              <div>
                <span className="text-blue-600">TF-IDF Documents:</span>
                <span className="ml-2 font-semibold">{searchAnalytics.documentsInTfIdf}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="text-center py-16">
            <div className="inline-flex items-center space-x-2">
              ...
              <span className="text-lg text-gray-600">AI is searching for the best results...</span>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((product) => (
              <div key={product.id} className="relative">
                ...
                {/* AI Score Indicator */}
                {product.aiScore && (
                  <div className="absolute top-2 right-2 bg-blue-500 text-white px-2 py-1 rounded-full text-xs font-semibold flex items-center space-x-1">
        ...
        {!loading && products.length === 0 && query && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-6">
              Our AI couldn't find any products matching "{query}". Try adjusting your search terms or filters.
            </p>
